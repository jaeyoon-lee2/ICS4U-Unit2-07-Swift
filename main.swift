...
do {
  ...
  let myTruck = Truck(licensePlateNumber: licensePlateNumber,
                          colour: colour, maxSpeed: truckMaxSpeed)
  ...
  while(true) {
    ...
    } else if (inputString == "CADENCE" && vehicleType == "Bike") {
      print("Enter the new cadence (\(myBike.GetCadence()) -> ?): ",
            terminator: "")
      guard let newCadence = Int(readLine()!) else {
        throw InvalidInputError.invalidInput
      }
      myBike.SetCadence(newCadence: newCadence)
